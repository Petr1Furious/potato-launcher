import type { ConfigOption, ConfigOptionKey, ContentRule } from "@/types/api";
...
export const defaultContentRule = (): ContentRuleForm => ({
  path: "",
...
export const parseKeyPath = (keyPath: string): ConfigOptionKey => {
  ...
  return trimmed.split(".").map((segment) => {
    ...
    return Number.isInteger(index) && segment === String(index)
      ? index
      : segment;
  });
};
...
export const configOptionsToForm = (
...
export const configOptionsFromForm = (
