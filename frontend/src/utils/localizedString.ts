...
export function isLocalizedMap(value: LocalizedString): value is Record<string, string> {
  return typeof value === 'object' && value !== null;
}

export function localizedStringToMode(value?: LocalizedString): LocalizedStringMode {
  if (!value) {
    return 'plain';
  }
...
export function localizedMapToEntries(map: Record<string, string>): LocalizedEntry[] {
  return Object.entries(map).map(([language, text]) => ({ language, text }));
}

export function entriesToLocalizedMap(entries: LocalizedEntry[]): Record<string, string> | undefined {
