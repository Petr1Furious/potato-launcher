...
const emit = defineEmits<{
  (
    event: "updated",
    payload: { id: string; data: Partial<InstanceResponse> },
  ): void;
...
const toEditableFields = (instance: InstanceResponse): EditableFields => ({
  ...
  mod_sync: {
    ...
    optional_sets:
      instance.mod_sync.optional_sets?.map((set) => ({
        ...set,
        mod_ids: [...set.mod_ids],
      })) ?? [],
  },
...
const handleUpdate = async () => {
  ...
  const optionalSetErrors = validateOptionalModSetIds(
    editData.mod_sync.optional_sets,
  );
...
const filebrowserUrl = computed(
  () => `/filebrowser/files/${props.instance.id}`,
);

const formatRuleFlags = (
  rule: NonNullable<InstanceResponse["content_rules"]>[number],
) => {
  const flags: string[] = [];
...
<template>
  <div class="space-y-6 p-4">
    <Card>
      <CardHeader>
        ...
        >
          <div>
            <CardTitle>{{
              isEditing ? "Edit Instance" : props.instance.id
            }}</CardTitle>
            <CardDescription>
              {{
                isEditing
          ...
          <dl class="grid gap-4 sm:grid-cols-2">
            ...
            <div v-if="props.instance.display_name" class="sm:col-span-2">
              ...
              <dd v-else class="text-sm font-medium">
                <ul class="space-y-1">
                  <li
                    v-for="(text, language) in props.instance.display_name"
                    :key="language"
                  >
            ...
            <div v-if="props.instance.default_xmx">
              ...
              <dd class="text-sm font-medium">
                {{ props.instance.default_xmx }}
              </dd>
            </div>
            <div>
              <dt class="text-sm">Mod Sync</dt>
              <dd class="text-sm font-medium">
                {{ props.instance.mod_sync.mode }}
              </dd>
            </div>
            <div>
              <dt class="text-sm">Resource Sync</dt>
              <dd class="text-sm font-medium">
                {{ props.instance.resource_sync }}
              </dd>
            </div>
            <div
              v-if="
                props.instance.auth_backend.type === AuthType.TELEGRAM &&
                props.instance.auth_backend.auth_base_url
              "
              class="sm:col-span-2"
            >
            ...
            <template
              v-if="props.instance.auth_backend.type === AuthType.ELY_BY"
            >
              <div>
                <dt class="text-sm">Client ID</dt>
                <dd class="text-sm font-medium wrap-break-word">
                  {{ props.instance.auth_backend.client_id }}
                </dd>
              </div>
              <div>
            ...
            <div
              class="sm:col-span-2"
              v-if="
                props.instance.content_rules &&
                props.instance.content_rules.length > 0
              "
            >
              ...
              <dd class="text-sm font-medium">
                <div class="border rounded-md divide-y">
                  <div
                    v-for="(rule, index) in props.instance.content_rules"
                    :key="index"
                    class="p-3 flex items-start justify-between gap-4"
                  >
                    <div>
                      <div
                        class="font-mono text-xs bg-muted px-1.5 py-0.5 rounded inline-block"
                      >
                        {{ rule.path }}
                      </div>
                    ...
                    <div
                      class="flex flex-wrap gap-2 text-xs text-muted-foreground justify-end"
                    >
                      <span
                        v-for="flag in formatRuleFlags(rule)"
                        :key="flag"
            ...
            <div
              class="sm:col-span-2"
              v-if="
                props.instance.mod_sync.optional_sets &&
                props.instance.mod_sync.optional_sets.length > 0
              "
            >
              ...
              <dd class="text-sm font-medium">
                <div class="border rounded-md divide-y">
                  <div
                    v-for="set in props.instance.mod_sync.optional_sets"
                    :key="set.id"
                    class="p-3"
                  >
                    <div class="font-medium">
                      ...
                      <ul v-else class="space-y-0.5">
                        <li
                          v-for="(text, language) in set.display_name"
                          :key="language"
                        >
                          <span
    ...
    <DeleteConfirmModal
      ...
      :instance-name="props.instance.id"
