...
const emit = defineEmits<{
  ...
  (
    event: "update-optional-set",
    index: number,
    field: "id" | "display_name" | "enabled_by_default",
    value: string | boolean | LocalizedString,
  ): void;
  (event: "update-optional-set-mod-ids", index: number, value: string): void;
}>();

const isVanillaLoader = computed(
  () => props.formData.mod_loader === LoaderType.VANILLA,
);
</script>
...
<template>
  <div class="space-y-6">
    <div class="space-y-5">
      <div class="grid gap-4 sm:grid-cols-2">
        <div class="space-y-2 sm:col-span-2">
          ...
          <Input
            ...
            :model-value="props.formData.id"
            :disabled="props.disabled"
            placeholder="Enter instance id"
            @update:modelValue="
              (value) => emit('update-field', 'id', value?.toString() ?? '')
            "
          />
          ...
          <p v-else class="text-sm text-muted-foreground">
            Lowercase slug: letters, digits, hyphens, and underscores. Must
            start with a letter.
          </p>
        </div>
        <div class="sm:col-span-2">
          <LocalizedStringEditor
            ...
            :disabled="props.disabled"
            @update:model-value="
              (value) => emit('update-field', 'display_name', value)
            "
          />
        </div>
        <div class="space-y-2">
          ...
          <Select
            :model-value="props.formData.minecraft_version || undefined"
          ...
          >
            ...
            <SelectContent>
              <SelectItem
                v-for="version in props.minecraftVersions"
                :key="version"
                :value="version"
              >
                {{ version }}
          ...
          <p
            v-if="props.errors?.minecraft_version"
            class="text-sm text-destructive"
          >
            {{ props.errors.minecraft_version }}
          </p>
          <p
            v-else-if="props.loadingMinecraftVersions"
        ...
        <div class="space-y-2">
          ...
          <Select
            :model-value="props.formData.mod_loader || undefined"
            :disabled="
              props.disabled ||
              ...
              (value) =>
                emit(
                  ...
                  (typeof value === 'string' && value.length
                    ? value
                    : LoaderType.VANILLA) as LoaderType,
                )
            "
          >
            <SelectTrigger>
            ...
            <SelectContent>
              <SelectItem
                v-for="loader in props.availableLoaders"
                :key="loader"
                :value="loader"
              >
                {{ loader }}
          ...
          <p
            v-else-if="!props.formData.minecraft_version"
            class="text-sm text-muted-foreground"
          >
            Select a Minecraft version first.
          </p>
          <p
            v-else-if="props.availableLoaders.length === 0"
            class="text-sm text-muted-foreground"
          >
        ...
        <div class="space-y-2">
          ...
          <Select
            :model-value="props.formData.loader_version || undefined"
            :disabled="
              props.disabled ||
            ...
            @update:modelValue="
              (value) =>
                emit('update-field', 'loader_version', value?.toString() ?? '')
            "
          >
            <SelectTrigger>
              <SelectValue placeholder="Select version" />
            ...
            <SelectContent>
              <SelectItem
                v-for="version in props.loaderVersions"
                :key="version"
                :value="version"
              >
                {{ version }}
          ...
          <p
            v-if="props.errors?.loader_version"
            class="text-sm text-destructive"
          >
            {{ props.errors.loader_version }}
          </p>
          <p
            v-else-if="!props.formData.mod_loader"
            class="text-sm text-muted-foreground"
          >
            Select a loader first.
          </p>
          <p
            v-else-if="!isVanillaLoader && props.loaderVersions.length === 0"
            class="text-sm text-muted-foreground"
          >
        ...
        <div class="space-y-2">
          ...
          <Select
            :model-value="props.formData.auth_backend.type"
            :disabled="props.disabled"
            @update:modelValue="
              (value) =>
                emit(
                  'update-auth-field',
                  'type',
                  (value as AuthType) ?? AuthType.OFFLINE,
                )
            "
          >
            <SelectTrigger>
        ...
        <div class="space-y-2 sm:col-span-2">
          <Label :for="`${props.idPrefix}-default-xmx`"
            >Default Xmx (RAM)</Label
          >
          <Input
            :id="`${props.idPrefix}-default-xmx`"
            :model-value="props.formData.default_xmx || ''"
            :disabled="props.disabled"
            placeholder="e.g. 4G or 4096M"
            @update:modelValue="
              (value) =>
                emit('update-field', 'default_xmx', value?.toString() ?? '')
            "
          />
          ...
          <p v-else class="text-sm text-muted-foreground">
            Optional. Used as the default JVM RAM limit (e.g.
            <span class="font-mono">4G</span>).
          </p>
        </div>
      </div>
      <div
        v-if="props.formData.auth_backend.type === AuthType.TELEGRAM"
        class="space-y-2"
      >
        <Label :for="`${props.idPrefix}-auth-base-url`">Auth Base URL *</Label>
        <Input
          :id="`${props.idPrefix}-auth-base-url`"
          type="url"
          :model-value="props.formData.auth_backend.auth_base_url || ''"
          :disabled="props.disabled"
      ...
      <div
        v-if="props.formData.auth_backend.type === AuthType.ELY_BY"
      ...
      >
        <div class="space-y-2">
          ...
          <Input
            ...
            :model-value="props.formData.auth_backend.client_id || ''"
            :disabled="props.disabled"
            placeholder="Ely.by client ID"
            @update:modelValue="
              (value) =>
                emit('update-auth-field', 'client_id', value?.toString() ?? '')
            "
          />
        ...
        <div class="space-y-2">
          <Label :for="`${props.idPrefix}-client-secret`"
            >Client Secret *</Label
          >
          <Input
            :id="`${props.idPrefix}-client-secret`"
            type="password"
            :model-value="props.formData.auth_backend.client_secret || ''"
            :disabled="props.disabled"
          ...
          />
          <p
            v-if="props.errors?.client_secret"
            class="text-sm text-destructive"
          >
    ...
    <div class="space-y-4">
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        <div class="space-y-2">
          <Label>Mode</Label>
          <Select
            :model-value="props.formData.mod_sync.mode"
            :disabled="props.disabled"
            @update:modelValue="
              (value) => emit('update-mod-sync-mode', value as ModSyncMode)
            "
          >
            <SelectTrigger>
              <SelectValue />
            ...
            <SelectContent>
              ...
              <SelectItem :value="ModSyncMode.MIRROR_FAST"
                >Mirror Fast</SelectItem
              >
            </SelectContent>
          </Select>
        </div>
        <div class="space-y-2 sm:col-span-2">
          <Label :for="`${props.idPrefix}-required-mods`"
            >Required Mod IDs</Label
          >
          <Input
            :id="`${props.idPrefix}-required-mods`"
            :model-value="
              props.modIdListToString(props.formData.mod_sync.required)
            "
            :disabled="props.disabled"
            placeholder="comma-separated mod ids"
            @update:modelValue="
              (value) =>
                emit('update-mod-id-list', 'required', value?.toString() ?? '')
            "
          />
        </div>
        <div class="space-y-2 sm:col-span-2">
          ...
          <Input
            :id="`${props.idPrefix}-blocked-mods`"
            :model-value="
              props.modIdListToString(props.formData.mod_sync.blocked)
            "
            :disabled="props.disabled"
            placeholder="comma-separated mod ids"
            @update:modelValue="
              (value) =>
                emit('update-mod-id-list', 'blocked', value?.toString() ?? '')
            "
          />
        </div>
      </div>
      <div class="space-y-3">
        ...
        <div
          v-if="!props.formData.mod_sync.optional_sets?.length"
          class="text-sm text-muted-foreground italic"
        >
          No optional mod sets defined.
        </div>
        <Card
          v-for="(set, index) in props.formData.mod_sync.optional_sets"
          :key="index"
        >
          <CardContent class="p-4 grid gap-4">
            <div class="grid gap-4 sm:grid-cols-2">
              <div class="space-y-2">
                ...
                />
                <p
                  v-if="props.errors?.[`optional_set_${index}_id`]"
                  class="text-sm text-destructive"
                >
            ...
            <label class="flex items-center gap-2 text-sm cursor-pointer">
              <input
                type="checkbox"
                :checked="set.enabled_by_default"
                :disabled="props.disabled"
            ...
            <div class="space-y-2">
              <Label>Mod IDs</Label>
              <Input
                :model-value="props.modIdListToString(set.mod_ids)"
                :disabled="props.disabled"
                placeholder="comma-separated mod ids"
                @update:modelValue="
                  (value) =>
                    emit(
                      'update-optional-set-mod-ids',
                      index,
                      value?.toString() ?? '',
                    )
                "
    ...
    <div class="space-y-2">
      ...
      <Select
        :model-value="props.formData.resource_sync"
        :disabled="props.disabled"
        @update:modelValue="
          (value) =>
            emit('update-field', 'resource_sync', value as ResourceSyncMode)
        "
      >
        <SelectTrigger>
          <SelectValue />
        ...
        <SelectContent>
          ...
          <SelectItem :value="ResourceSyncMode.ALWAYS_FAST"
            >Always Fast</SelectItem
          >
        </SelectContent>
      </Select>
    </div>
    ...
    <div class="space-y-4">
      ...
      <div
        v-if="!props.formData.content_rules?.length"
        class="text-sm text-muted-foreground italic"
      >
      ...
      <div v-else class="space-y-3">
        <Card
          v-for="(rule, index) in props.formData.content_rules"
          :key="index"
        >
          <CardContent class="p-4 grid gap-4">
            <div class="grid gap-4 sm:grid-cols-2">
              <div class="space-y-2 sm:col-span-2">
                <Label :for="`${props.idPrefix}-rule-${index}-path`"
                  >Path</Label
                >
                <Input
                  :id="`${props.idPrefix}-rule-${index}-path`"
                  :model-value="rule.path"
                  :disabled="props.disabled"
                  placeholder="e.g. config or options.txt"
                  @update:modelValue="
                    (value) =>
                      emit(
                        'update-content-rule',
              ...
              <div class="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem :value="ContentRuleType.DIRECTORY"
                      >Directory</SelectItem
                    >
                    <SelectItem :value="ContentRuleType.CONFIG_OPTIONS"
                      >Config Options</SelectItem
                    >
                  </SelectContent>
                </Select>
              </div>
              <div class="space-y-2">
                ...
                <Select
                  :model-value="rule.apply_on || ApplyOn.UPDATE"
                  :disabled="props.disabled"
                  @update:modelValue="
                    (value) =>
                      emit(
            ...
            <template v-if="rule.type === ContentRuleType.DIRECTORY">
              ...
              <div class="flex flex-wrap gap-6">
                <label class="flex items-center gap-2 text-sm cursor-pointer">
                  <input
                    type="checkbox"
                    :checked="rule.delete_extra ?? true"
                    :disabled="props.disabled"
                    class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary disabled:opacity-50"
                    @change="
                      (e) =>
                ...
                <label class="flex items-center gap-2 text-sm cursor-pointer">
                  <input
                    type="checkbox"
                    :checked="rule.skip_if_dir_exists ?? false"
                    :disabled="props.disabled"
                    class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary disabled:opacity-50"
                    @change="
                      (e) =>
              ...
              <p class="text-xs text-muted-foreground">
                Skip if dir exists ignores the rule when the directory already
                has a download-complete marker.
              </p>
            </template>
            <template v-if="rule.type === ContentRuleType.CONFIG_OPTIONS">
              <div class="space-y-2">
                ...
                <Select
                  :model-value="rule.config_type || ConfigType.JSON"
                  :disabled="props.disabled"
                  @update:modelValue="
                    (value) =>
                      emit(
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem :value="ConfigType.PROPERTIES"
              ...
              <div class="space-y-3">
                ...
                <div
                  v-for="(option, optionIndex) in rule.optionsForm"
                  :key="optionIndex"
