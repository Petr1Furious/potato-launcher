...
const loadInstances = async () => {
  ...
  try {
    ...
    if (selectedInstance.value) {
      const exists = instances.value.some(
        (m) => m.id === selectedInstance.value,
      );
      if (!exists) {
...
const handleInstanceUpdate = (payload: {
  id: string;
  data: Partial<InstanceResponse>;
}) => {
  ...
  <div class="flex min-h-screen">
    ...
    <main class="flex-1 p-8">
      ...
      <div v-else>
        ...
        <InstanceDetails
          v-else-if="selectedInstanceData"
          :key="selectedInstanceData.id"
          :instance="selectedInstanceData"
          @updated="handleInstanceUpdate"
          @deleted="handleInstanceDelete"
        />
        <div v-else class="flex items-center justify-center min-h-[60vh] p-4">
          <Card class="w-full max-w-xl">
            <CardHeader class="text-center">
              <CardTitle class="text-2xl"
                >Welcome to Instance Manager</CardTitle
              >
              <CardDescription>
                Select an instance from the sidebar or create a new one to get
                started.
              </CardDescription>
            </CardHeader>
